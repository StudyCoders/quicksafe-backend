...
if (verificarPropriedades($dados, $propriedades)) {
    ...
    retorna_erro("Informe todas as propriedades necessárias: tipo_contato e id_contato (caso tenha).", 400);
}

if(!$contato_existe) {
    $sql = "INSERT INTO CONTATOS(TIPO_CONTATO, ID_USUARIO)
                VALUES
            (?, ?)";
    $values = array($tipo_contato, $id_usuario);
    ...
    $sql = "UPDATE CONTATOS SET
                TIPO_CONTATO = ?
            WHERE ID_CONTATO = ?";
    $values = array($tipo_contato, $id_contato);
